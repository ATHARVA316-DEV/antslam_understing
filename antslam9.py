...
class Ant:
    ...
    def move(self):
        ...
        options = []
        pheromone_levels = []
        for n in self.neighbors(self.pos):
        ...
        if options:
            if self.carrying_food:
                ...
                nest = self.env.nest
                options.sort(key=lambda p: abs(p.x - nest.x) + abs(p.y - nest.y))
                next_pos = options[0]
            else:
                ...
                next_pos = random.choices(options, weights=probs, k=1)[0]
        else:
        ...
        self.speed_history.append(1 if next_pos != self.pos else 0)
